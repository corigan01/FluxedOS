 ...
 * */
gtree_t * tree_create() {
    return (gtree_t*)malloc(sizeof(gtree_t) * 1);
}
...
gtreenode_t * tree_find_parent_recur(gtree_t * tree, gtreenode_t * remove_node, gtreenode_t * subroot, int * child_index) {
    ...
    foreach(child, subroot->children) {
        gtreenode_t * ret = tree_find_parent_recur(tree, remove_node, (gtreenode_t*)child->val, child_index);
        if(ret != NULL) {
            return ret;
        }
...
void tree2array_recur(gtreenode_t * subroot, void ** array, int * size) {
    ...
    *size = *size + 1;
    foreach(child, subroot->children) {
        tree2array_recur((gtreenode_t*)child->val, array, size);
    }
}
