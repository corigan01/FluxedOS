...
void vfs_db_listdir(char * name) {
    ...
    if(!n) {
        VGA::PRINT_STR("Could not list a directory that does not exist\n");
        return;
    }
    if(!n->listdir) return;
...
void print_vfstree_recur(gtreenode_t * node, int parent_offset) {
    ...
    foreach(child, node->children) {
        print_vfstree_recur((gtreenode_t*)child->val, parent_offset + len + 1);
    }
}
/*
 ...
 * */
void vfs_init() {
    VGA::PRINT_STR("VFS INIT ");
    vfs_tree = tree_create();
    struct vfs_entry * root = (vfs_entry*)malloc(sizeof(struct vfs_entry));
...
void vfs_mount_recur(char * path, gtreenode_t * subroot, vfs_node_t * fs_obj) {
    ...
    if(!found) {
         ...
         struct vfs_entry * ent = (vfs_entry*)malloc(sizeof(struct vfs_entry)* 1);
