...
void ATA::read_sectors_ATA_PIO(uint32_t *target_address, uint32_t LBA, uint8_t sector_count)
{
...
void ATA::write_sectors_ATA_PIO(uint32_t LBA, uint8_t sector_count, uint32_t* bytes)
{
	ATA_wait_BSY();
	port_byte_out(0x1F6,0xE0 | ((LBA >>24) & 0xF));
	port_byte_out(0x1F2,sector_count);
...
static void ATA::ATA_wait_BSY()   //Wait for bsy to be 0
{
	while(port_byte_in(0x1F7)&STATUS_BSY);
}
static void ATA::ATA_wait_DRQ()  //Wait fot drq to be 1
{
