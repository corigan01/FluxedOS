...
uint32_t ext2_file_size(vfs_node_t * node) {
    ext2_fs_t * ext2fs = (ext2_fs_t *)node->device;
...
void ext2_mkdir(vfs_node_t * parent, char * name, uint16_t permission) {
    ...
    inode_t * p_inode = (inode_t *)malloc(sizeof(inode_t));
...
char ** ext2_listdir(vfs_node_t * parent) {
    ...
    while(curr_offset < p_inode->size) {
        ...
        if(size + 1 == cap) {
            ret = (char **)malloc(sizeof(char*) * cap * 2);
            cap = cap * 2;
        }

        direntry_t * curr_dir = (direntry_t*)(block_buf + in_block_offset);
        if(curr_dir->inode != 0) {
            char * temp = (char *)malloc(curr_dir->name_len + 1);
...
vfs_node_t * vfsnode_from_direntry(ext2_fs_t * ext2fs, direntry_t * dir, inode_t * inode) {
    vfs_node_t * ret = (vfs_node_t *)malloc(sizeof(vfs_node_t) * 1);
    ...
    ret->open    = ext2_open;
    ret->close   = (close_callback)ext2_close;
    return ret;
}
...
void ext2_create_entry(vfs_node_t * parent, char * entry_name, uint32_t entry_inode) {
    ...
    char * check = (char*)malloc(entry_name_len + 1);
    char * block_buf = read_inode_block(ext2fs, p_inode, block_offset);
    ...
    while(curr_offset < p_inode->size) {
        ...
        if(curr_dir->name_len == entry_name_len) {
            ...
            if(curr_dir->inode != 0 && !strcmp(entry_name, check)) {
                VGA::PRINT_STR("Entry by the same name already exist\n");
                return;
            }
        }
...
void ext2_chmod(vfs_node_t * file, uint32_t mode) {
    ext2_fs_t * ext2fs = (ext2_fs_t *)file->device;
    inode_t * inode = (inode_t *)malloc(sizeof(inode_t));
...
void read_inode_metadata(ext2_fs_t * ext2fs, inode_t * inode, uint32_t inode_idx) {
    ...
    uint32_t group = (inode_idx + 1) / (ext2fs->inodes_per_group + 1);
    ...
    uint32_t block_offset = (idx_in_group - 1) * (ext2fs->sb->inode_size + 1) / (ext2fs->block_size + 1);
    // Offset within block
    uint32_t offset_in_block = (idx_in_group - 1) - block_offset * ((ext2fs->block_size + 1) / (ext2fs->sb->inode_size + 1));
    char * block_buf = (char*)malloc(sizeof(ext2fs->block_size) * 4096);
...
char * read_inode_block(ext2_fs_t * ext2fs, inode_t * inode, uint32_t iblock) {
    char * buf = (char*)malloc(sizeof(ext2fs->block_size) * 4096);
...
void rewrite_bgds(ext2_fs_t * ext2fs) {
    for(uint32_t i = 0; i < ext2fs->bgd_blocks; i++)
        write_disk_block(ext2fs, 2, (char*)ext2fs->bgds + i * ext2fs->block_size);
}

void rewrite_superblock(ext2_fs_t * ext2fs) {
    write_disk_block(ext2fs, 1, (char*)ext2fs->sb);
}
...
int alloc_inode_metadata_block(uint32_t * block_ptr, ext2_fs_t * ext2fs, inode_t * inode, uint32_t inode_idx, char * buffer, unsigned int block_overwrite) {
    if(!(*block_ptr)) {
        ...
        *block_ptr = block_no;
        if(buffer)
            write_disk_block(ext2fs, block_overwrite, (char*)buffer);
        else
            write_inode_metadata(ext2fs, inode, inode_idx);
        return 1;
    }
...
uint32_t get_disk_block_number(ext2_fs_t * ext2fs, inode_t * inode, uint32_t inode_block) {
    ...
    int a, b, c, d, e, f, g;
    unsigned int * tmp = (uint32 *)malloc(sizeof(ext2fs->block_size) * 4096);
    ...
    b = a - p;
    if(b < 0) {
        read_disk_block(ext2fs, inode->blocks[EXT2_DIRECT_BLOCKS], (char*)tmp);
        ret = tmp[a];
        goto done;
    }
    c = b - p * p;
    if(c < 0) {
        c = b / p;
        d = b - c * p;
        read_disk_block(ext2fs, inode->blocks[EXT2_DIRECT_BLOCKS + 1], (char*)tmp);
        read_disk_block(ext2fs, tmp[c], (char*)tmp);
        ret = tmp[d];
        goto done;
    }
    d = c - p * p * p;
    if(d < 0) {
        ...
        f = (c - e * p * p) / p;
        g = (c - e * p * p - f * p);
        read_disk_block(ext2fs, inode->blocks[EXT2_DIRECT_BLOCKS + 2], (char*)tmp);
        read_disk_block(ext2fs, tmp[e], (char*)tmp);
        read_disk_block(ext2fs, tmp[f], (char*)tmp);
        ret = tmp[g];
        goto done;
    }
done:
...
void set_disk_block_number(ext2_fs_t * ext2fs, inode_t * inode, uint32_t inode_idx, uint32_t inode_block, uint32_t disk_block) {
    ...
    if(b <= 0) {
        ...
        write_disk_block(ext2fs, inode->blocks[EXT2_DIRECT_BLOCKS], (char*)tmp);
        tmp[a] = disk_block;
        goto done;
    }
    c = b - p * p;
    if(c <= 0) {
        ...
        read_disk_block(ext2fs, inode->blocks[EXT2_DIRECT_BLOCKS + 1], (char*)tmp);
        if(!alloc_inode_metadata_block(&(tmp[c]), ext2fs, inode, inode_idx, (char*)tmp, inode->blocks[EXT2_DIRECT_BLOCKS + 1]));
        unsigned int temp = tmp[c];
        read_disk_block(ext2fs, temp, (char*)tmp);
        tmp[d] = disk_block;
        write_disk_block(ext2fs, temp, (char*)tmp);
        goto done;
    }
    d = c - p * p * p;
    if(d <= 0) {
        ...
        read_disk_block(ext2fs, inode->blocks[EXT2_DIRECT_BLOCKS + 2], (char*)tmp);
        if(!alloc_inode_metadata_block(&(tmp[e]), ext2fs, inode, inode_idx, (char*)tmp, inode->blocks[EXT2_DIRECT_BLOCKS + 2]));
        unsigned int temp = tmp[e];
        read_disk_block(ext2fs, tmp[e], (char*)tmp);
        if(!alloc_inode_metadata_block(&(tmp[f]), ext2fs, inode, inode_idx, (char*)tmp, temp));
        temp = tmp[f];
        read_disk_block(ext2fs, tmp[f], (char*)tmp);
        tmp[g] = disk_block;
        write_disk_block(ext2fs, temp, (char*)tmp);
...
uint32_t ext2_alloc_block(ext2_fs_t * ext2fs) {
    uint32_t * buf = (uint32*)malloc(sizeof(ext2fs->block_size) * 4096);
    ...
    for(uint32_t i = 0; i < ext2fs->total_groups; i++) {
        ...
        read_disk_block(ext2fs, bitmap_block, (char*)buf);
        for(uint32_t j = 0; j < ext2fs->block_size / 4; j++) {
            ...
            for(uint32_t k = 0; k < 32; k++) {
                ...
                if(free) {
                    ...
                    uint32_t mask = (0x1 << k);
                    buf[j] = buf[j] | mask;
                    write_disk_block(ext2fs, bitmap_block, (char*)buf);
                    // update free_inodes
...
vfs_node_t * get_ext2_root(ext2_fs_t * ext2fs, inode_t * inode) {
    vfs_node_t * ext2root = (vfs_node_t*)malloc(sizeof(vfs_node_t) * 1);
    strcpy_(ext2root->name, "/");
    ...
    ext2root->close   = (close_callback)ext2_close;
...
void ext2_init(char * device_path, char * mountpoint) {
    VGA::PRINT_STR("EXT INIT ");



    // First, we need to store some information about the ext2-formatted disk
    ext2_fs_t * ext2fs = (ext2_fs_t *)malloc(sizeof(ext2_fs_t) * 1);
    ext2fs->disk_device= file_open(device_path, 0);
    ext2fs->sb = (superblock_t*)malloc(SUPERBLOCK_SIZE);
    ...
    read_disk_block(ext2fs, 1, (char*)ext2fs->sb);
    ...
    ext2fs->total_groups = (ext2fs->sb->total_blocks + 1) / (ext2fs->blocks_per_group + 1);
    ...
    ext2fs->bgds = (bgd_t*)malloc(sizeof(bgd_t) * ext2fs->bgd_blocks * ext2fs->block_size);
    for(uint32_t i = 0; i < ext2fs->bgd_blocks; i++) {
        read_disk_block(ext2fs, 2, (char*)ext2fs->bgds + i * ext2fs->block_size);
    }
    ...
    inode_t * root_inode = (inode_t*)malloc(sizeof(inode_t) * 1);
